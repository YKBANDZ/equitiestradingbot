...
class TradingBot:
    ...
    def start(self, single_pass=False) -> None:
        ...
        while True:
            try:
                ...
                # Now process markets from the configured markets source
    ...
    def safety_checks(self) -> None:
        """
        Perform some safety checks before running the strategy against the next 
        market
        ...
        if percent_used is None:
            logging.warning(
                "Stop trading because account percentage used can't be fetched"
            )
    ...
    def process_trade(
        ...
        for item in open_positions:
            ...
            if item.epic == market.epic and direction is item.direction:
                logging.info(
                    "The is already an open position for the epic, skip trade"
                )
                return
