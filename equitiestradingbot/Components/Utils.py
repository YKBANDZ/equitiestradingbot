...
class MarketClosedException(Exception): 
    """Error to notify that the market is closed"""
...
class Utils:
    """
    Utility class containing stactic methods to perform simple general actions
    """
    ...
    def percentage(
        ...
        """Return the percentage value of the part of the whole"""
        return 100 * float(part) / float(whole)
    
    @staticmethod
    def is_between(time: str, time_range: Tuple[str, str]):
        """Return True if the time is between the time_range. time must be a string.
        time_range msust be a tuple (a.b) where a and b are strings in formate 'HH:MM'"""
