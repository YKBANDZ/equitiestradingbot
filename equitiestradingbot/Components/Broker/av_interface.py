...
from .broker import StocksInterface
...
class AVInterface(StocksInterface):
    ...
    def daily(self, marketId: str) -> pandas.DataFrame:
        """
        Calls AlphaVantagae API and return the Daily time series for the
        given market
            - **marketId**: string representing an AlphaVantage compatible market id
            - Returns **None** if an error occurs otherwise the pandas datraframe
        """
        self._wait_begfore_call(self._config.get_alphavantage_api_timeout())
    ...
    def _format_market_id(self, marketId: str) -> str:
        ...
        Adds the marekt exchange prefit (i.e. London is LON:)
        """
