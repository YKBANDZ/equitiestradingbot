...
class SignalManager:
    def __init__(self, config: Configuration):
        self.config = config
        self.signal_window = float(self.config.get_telegram_signal_window()) * 3600  # Convert to seconds
    ...
    def _load_history(self):
        ...
        try:
            with Path(self.config.get_telegram_signal_history_filepath()).open(mode='r') as f:
    ...
    def _save_history(self):
        ...
        try:
            ...
            Path(self.config.get_telegram_signal_history_filepath()).parent.mkdir(parents=True, exist_ok=True)
            
            with Path(self.config.get_telegram_signal_history_filepath()).open(mode='w') as f:
    ...
    def can_send_signal(self, market_epic: str) -> bool:
        """Check if enough time has passed to send a new signal"""
        current_time = time.time()
        last_signal_time = self.signal_timestamps.get(market_epic, 0)
        
        if current_time - last_signal_time >= self.signal_window:
            return True
        logging.debug(f"Signal for {market_epic} skipped - last signal was {current_time - last_signal_time:.0f} seconds ago")
        return False
    ...
    def record_signal(self, market_epic: str):
        ...
        self._save_history()
        logging.info(f"Recorded new signal for {market_epic}") 