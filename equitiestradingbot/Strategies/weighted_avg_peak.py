...
class WeightedAvgPeak(Strategy):
    ...
    def read_configuration(self, config: Configuration) -> None:
        ...
        self.too_high_margin = 100 #No crazy high pip limit per trade
        # Normally would be 3/22 days but stocks require a lower multiplier
        self.ce_multiplier = 2
        self.greed_indicator = 99999

    def initialise(self) -> None:
        """
        Initiailise strategy
        """
        pass
    ...
    def find_trade_signal(
        ...
        # a stock price moves below VWAP for a given time period or initiate a 
        # long position is stock prices moves above VWAP
        ...
        # how many "peaks" are ABOVE threshold
    ...
    def calculate_stop_loss(
        ...
        TR_prices = []
        # They should all be the same length but just in case to be safe
        ...
        for index in range(length):
            if first_time_round_loop:
                ...
                closing_prices.apped(closePrice)
    ...
    def peakdet(
        ...
        %       MAXTAB and MINTAB consists of two columns. Colums 1
        %       contains indices in V, and column 2 the found values.
        %
        %       With [MAXTAB, MINTAB] = PEAKDET(V, DELTA, X) the indices
        %       in MAXTAB and MINTAB are replaced with the correspinding
        %       X-values.
        %
        %       A point is considered a maximum peak if it has the maximal
        %       value, and was preceded (to the left) by a value lower by DELTA.
        
        % Eli Billuer, 3.4.05 (not copyrighted).
        % This function is released to the public domain; Any use allowed.
        ...
        if len(v) != len(x):
            logging.error("Input vectors v and x must have the same length")
            return None, None

        if not isscalar(delta):
            logging.error("Input argument delta must be scalar")
            return None, None
