...
from equitiestradingbot.interfaces import Market, MarketHistory, MarketMACD, Position
...
def test_get_prices(broker):
    ...
    assert MarketHistory.HIGH_COLUMN_COLUMN in hist.dataframe
    ...
    assert MarketHistory.VOLUE_COLUMN in hist.dataframe
    assert len(hist.dataframe[MarketHistory.VOLUE_COLUMN]) > 0


def test_get_macd(broker):
    market = broker.get_market_inf("mock")
