...
from ...equitiestradingbot.components.broker.ig_interface import IG_API_URL

#Setting global variables and used in fixtures
...
def ig_request_market_info(mock, args="", data="mock_market_info.json", fail=False):
    ...
    mock.get(
        re.compile("{}/{}/{}".format(IG_BASE_URI, IG_API_URL.MARKET.value, args)),
        json=data
...
def ig_request_search_market(mock, args="", data="mock_market_search.json", fail = False):
    ...
    mock.get(
        re.compile(
            re.escape(
                "{}/{}?searchTerm={}".format( 
                    IG_BASE_URI, IG_API_URL.MARKET.value, args
                )
            )
        ),
        json=read_json("{}/{}".format(TEST_DATA_IG, data)),
...
def ig_request_prices(mock, args="", data="mock_historic_prices.json", fail=False):
    """Mock prices call"""
    mock.get(
