...
from equitiestradingbot.components import Configuration, MarketProvider
...
def config():
    ...
    """
    return Configuration.from_filepath(Path("test/test_data/demo_trading_bot.toml"))
...
def test_market_provider_epics_list(config, broker):
    ...
    """
    # Configure Equitiestradingbot to use an epic list
    ...
    for _ in range(4):
        ...
        # Keep calling the test function building a list of returned epics
        actual_list = []
        try:
            while True:
            ...
            # Verift reading teh next raise another exception
            with pytest.raises(StopIteration):
                mp.next()
            mp.reset()
            continue
        # If we get here it measn that next did not raise an exception at the end of the list
        assert False


def test_market_provider_watchlist(config, broker):
    """
    Test the MarketProvider configured to fetch markets from IGWatchlist
    """
    ...
    # data for the watchlist contain markets
    # Run the test several times resetting the market provider
    for _ in range(4):
        assert mp.next().epic == "UD.D.TSLA.CASH.IP"
        assert mp.next().epic == "UD.D.TSLA.CASH.IP"
        assert mp.next().epic == "UD.D.TSLA.CASH.IP"
...
def test_market_provider_market_from_epic(config, broker):
    ...
    config.config["market_source"]["epic_id_list"][
        "filepath"
    ] = "test/test_data_epics_list.txt"

    # create class to test
    mp = MarketProvider(config, broker)
    market = mp.get_market_from_epic("mock")
    assert market is not None
    assert market.epic == "UD.D.TSLA.CASH.IP"
...
def test_search_market(config, broker, requests_mock):
    ...
    # The market search data contains multiple markets
