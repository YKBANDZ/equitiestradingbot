...
from equitiestradingbot.components import Configuration
...
def test_value_getters():
    config = Configuration.from_filepath(Path("Documents/equitiestradingbot/test/test_data/demo_trading_bot.toml"))
    assert config is not None
    assert config.get_max_account_usable() == 90
    assert config.get_time_zone() == "Europe/London"
    assert config.get_credentials_filepath() == "{home}/Documents/equitiestradingbot/test/test_data/demo_credentials.json"
    ...
    assert "{home}/Documents/equitiestradingbot/log/trading_bot" in config.get_log_filepalth()
    assert "{timestamp}" not in config.get_log_filepalth()
    ...
    assert config.get_epic_ids_filepath() == "{home}/Documents/equitiestradingbot/data/epic_ids.txt"
    assert config.get_watchlist_name() == "equitybot"
...
def test_replace_placeholders():
    mock_dict = {
        ...
        "key_list": ["string1", "string2"],
        "key_dict": {
            "subkey1": "{home}/Documents/equitiestradingbot",
            "subkey2": [1, 2, 3],
