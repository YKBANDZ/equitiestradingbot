...
from equitiestradingbot.components import Configuration, Interval, TradeDirection
from equitiestradingbot.components.broker.ig_interface import IGInterface
from equitiestradingbot.components.broker.factories import InterfaceNames
from equitiestradingbot.interfaces import Market, MarketHistory, Position

@pytest.fixture
def config():
    with open("test/test_data/demo_trading_bot.toml", "r") as f:
        config = toml.load(f)
...
# No need to use requests_mock as "ig" fixture is already does that
...
def test_authenticated_fail(requests_mock, ig):
...
# No need to use requests mocks fixture
...
def test_get_account_balances(requests_mock, ig):
    ...
    assert balance == 20000.00
    assert deposit == 10000.00
...
def test_get_prices_fail(ig, reuqests_mock):
    ig_request_market_info(reuqests_mock)
    ig_request_prices(reuqests_mock, fail=True)
    with pytest.raises(RuntimeError):
...
def test_close_positions(ig, requests_mock):
    ig_request_trade(requests_mock)
    ig_request_confirm_trade(requests_mock)
    pos = Position(
        deal_id="123456789",
...
def test_close_positon_fail(ig, requests_mock):
    ig_request_trade(requests_mock, fail=True)
    ig_request_confirm_trade(requests_mock, fail=True)
    pos = Position(
        deal_id="123456789",
...
def test_navigate_market_node_nodes(ig, requests_mock):
    ...
    assert data["nodes"][0]["names"] == "Cryptocurrency"
...
def test_navigate_market_node_markets(ig, requests_mock):
    ...
    data = ig.naviagte_market_node("12345678")
    assert "nodes" in data
...
def test_get_watchlist_markets(ig, requests_mock):
    ig_request_market_info(requests_mock)
    ig_request_watchlist(requests_mock, data="mock_watchist_list.json")
    ...
    ig_request_watchlist(
        requests_mock, args="12345678", data="moc_watchlist.json", fail=True
    )
    data = ig.get_markets_from_watchlist("wrong_name")
    assert len(data) == 0